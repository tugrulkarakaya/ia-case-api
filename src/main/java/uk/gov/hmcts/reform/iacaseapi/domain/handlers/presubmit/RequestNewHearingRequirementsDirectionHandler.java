...
public class RequestNewHearingRequirementsDirectionHandler implements PreSubmitCallbackHandler<AsylumCase> {
    ...
    protected void writePreviousHearingsToAsylumCase(AsylumCase asylumCase) {
        ...
        final Optional<String> allocatedJudge = asylumCase.read(ALLOCATED_JUDGE, String.class);

        final String appellantNameForDisplay = asylumCase.read(APPELLANT_NAME_FOR_DISPLAY, String.class)
            .orElseThrow(() -> new IllegalStateException("appellantNameForDisplay is missing."));

        final String respondentRepresentative = asylumCase.read(RESPONDENT_REPRESENTATIVE, String.class)
            .orElseThrow(() -> new IllegalStateException("respondentRepresentative is missing."));
